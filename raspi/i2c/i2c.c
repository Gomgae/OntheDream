...
// Note timing changes for new version of delayMicroseconds ()
  ...
  lcd_digitalWrite (lcd->strbPin, 1) ; delayMicroseconds (50) ;
  lcd_digitalWrite (lcd->strbPin, 0) ; delayMicroseconds (50) ;
}
...
static void sendDataCmd (const struct lcdDataStruct *lcd, unsigned char data)
{
  ...
  if (lcd->bits == 4)
  {
    ...
    for (i = 0 ; i < 4 ; ++i)
    {
      lcd_digitalWrite (lcd->dataPins [i], (d4 & 1)) ;
      d4 >>= 1 ;
    }
    strobe (lcd) ;
  ...
  else
  {
    for (i = 0 ; i < 8 ; ++i)
    {
      lcd_digitalWrite (lcd->dataPins [i], (myData & 1)) ;
      myData >>= 1 ;
    }
  }
  strobe (lcd) ;
...
static void put4Command (const struct lcdDataStruct *lcd, unsigned char command)
{
  ...
  for (i = 0 ; i < 4 ; ++i)
  {
    lcd_digitalWrite (lcd->dataPins [i], (myCommand & 1)) ;
    myCommand >>= 1 ;
  }
  strobe (lcd) ;
}
...
//////////////////////////////////
int i2c_lcdInit(int addr)
{
  int fd2 = -1;
  yang = wiringPiI2CSetup (addr);
  if (yang < 0){
    return -1;
  }
  
  //int wiringPiI2CWriteReg8 (int fd, int reg, int data) ;
  //  wiringPiI2CWriteReg8 (yang, 0x0a, 0x84) ;  // IOCON - set BANK bit
  //  wiringPiI2CWriteReg8 (yang, 0x05, 0x84) ;  // IOCON - set ODR in bank 0
  //  wiringPiI2CWriteReg8 (yang, 0x00, 0x00) ;  // Port A -> Outputs
  wiringPiI2CWrite (yang, 0x84) ;  // IOCON - set BANK bit
  wiringPiI2CWrite (yang,  0x84) ;  // IOCON - set ODR in bank 0
  wiringPiI2CWrite (yang,  0x00) ;  // Port A -> Outpus

 //Initialise LCD(int rows, int cols, int bits, int rs, int enable, int d0, int d1, int d2, int d3, int d4, int d5, int d6, int d7)
 //                 ?됯컻???닿컻??鍮꾪듃??? RS?, ENABLE?, ?곗씠??,2,3,4 , (4bit 紐⑤뱶?꾩쑝濡??ㅼ뿉4媛쒕뒗 ?앸왂)

  fd2 = lcdInit(2, 16, 4, 0,2, 4,5,6,7, 0,0,0,0);
  return fd2;
...
// Create a new LCD:
  ...
  lcd_digitalWrite (lcd->rsPin,   0) ; //pinMode (lcd->rsPin,   OUTPUT) ; 誘몄궗???묓쁽??
  lcd_digitalWrite (lcd->strbPin, 0) ; //pinMode (lcd->strbPin, OUTPUT) ;

  for (i = 0 ; i < bits ; ++i)
  {
    lcd_digitalWrite (lcd->dataPins [i], 0) ;
...
void lcd_digitalWrite(int pin, int value)
{
  ...
  oldval &= ~(1 << pin);
  oldval |= value << pin;
  wiringPiI2CWrite (yang, oldval) ;
 // wiringPiI2CWriteReg8 (yang, 0x0a, oldval) ;
}
